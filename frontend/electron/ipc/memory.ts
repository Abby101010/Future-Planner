...
import { reflectInWorker } from "../reflection-worker-client";
...
export function registerMemoryIpc(): void {
  ...
  // Routed through reflection-worker-client so the 2-5s Claude call runs
  // in a worker thread instead of freezing the Electron main thread.
  ipcMain.handle("memory:reflect", async (_event, payload) => {
    try {
      ...
      const result = await reflectInWorker(
        payload?.trigger || "manual",
        apiKey,
      );

      // The worker mutated memory state in its own V8 isolate. Refresh
      // the main-process cache from disk so subsequent reads see the
      // new facts/preferences.
      try {
        await ensureManagerReady();
        loadMemory();
      } catch (err) {
        console.warn("[memory:reflect] post-reflect refresh failed:", err);
      }
