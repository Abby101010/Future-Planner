...
Last updated: 2026-04-26 (latest) — **Chat is conversational. AI mutations require explicit user confirmation. Watchdog on Opus calls.**

⚠ **CONTRACT CHANGE — read before debugging the chat flow:**

(1) **`pending_actions` table (migration 0017)** — new audit + queue for AI-proposed mutations. Status `pending | accepted | rejected | expired`. Default 24h TTL. RLS scoped per user.

(2) **`STARWARD_CHAT_AUTO_DISPATCH`** — opt-OUT env var. **Default = cards required.** When unset, every AI-emitted intent from `/ai/home-chat/stream` and `/ai/chat/stream` is written to `pending_actions` and the SSE `done` payload returns `intents: []`. Set to `1` only for emergency rollback.

(3) **`command:accept-pending-action { actionId }`** — runs the underlying intent's command via inline switch on `intent_kind`. Marks the row `accepted`. Currently dispatches: `manage-task` (complete/skip/delete/reschedule), `manage-reminder` (acknowledge), `create-task`. Add new `intent_kind` cases as the AI's vocabulary grows. Invalidates `view:dashboard|tasks|calendar|planning|goal-plan` (conservative — could touch any).

(4) **`command:reject-pending-action { actionId, reason? }`** — marks the row `rejected`. **No state mutation. Chat session continues.** The rejection (and reason) is queryable via `pendingActionsRepo.listRecentRejectionsForSession(sessionId)` so the AI's next turn can have a conversational follow-up. Invalidates only `view:dashboard|tasks` (clears the proactive nudge).

(5) **`view:tasks.pendingActions[]` and `view:dashboard.pendingActions[]`** — array of `PendingAction` ({id, intentKind, intentPayload, proposedSummary, status, proposedAt, expiresAt, sessionId, ...}). Populated from `pendingActionsRepo.listActive()`. Each pending action also writes a `proactive` nudge `pending-action:${id}` so v0.1.32 readers see "AI proposal: <summary>" in the existing NotifStack body.

(6) **SSE `done` payload now includes `pendingActionIds: string[]`** — IDs of pending_actions created by this turn. The next FE release reads these to render Accept/Reject cards keyed to the proposals just made. Empty array when `STARWARD_CHAT_AUTO_DISPATCH=1` is set.

(7) **`runStreamingHandler` watchdog** — every Opus / Sonnet / Haiku call now races against a 120s timeout (configurable per-handler via `timeoutMs`). On timeout, throws `chat_timeout: <handlerKind> did not complete within 120000ms` so the SSE error path runs and the FE's `finally` clears `streaming`. Backstops the "stuck pending chat" symptom even when the FE's own `postSseStream` lacks a timeout.

**Behavioral consequence for v0.1.32:** `dispatchChatIntent` (FE) now receives `intents: []` and has nothing to auto-dispatch. AI proposals are visible as `proactive` nudges. Until the next FE release renders Accept/Reject cards, the user can accept via dev harness (`POST /commands/accept-pending-action {actionId}`) or by acting through the existing in-app buttons (Refresh, Add task, Reschedule cards) which work as before — only the *chat* path changed.

Last updated: 2026-04-26 (later) — **Plan-overhaul detection + classify command, manual-task must-do default, swap-suggestion in overwhelm nudge, deterministic auto-scheduler in mutation pipeline.**

⚠ NEW WIRE SHAPES (2026-04-26 PM):

(1) **`command:plan-edit-classify`** added — body `{ goalId, proposedPlan: GoalPlan }`. Read-only; computes diff against current persisted plan and returns `{ diff, projectedDailyTasksAffected, recommendation }`. Lets a future FE preview "this change will reset N future tasks" before committing. Invalidations: none (read-only). No FE wiring yet.

(2) **`goalPlan.replacePlan` now returns `ReplacePlanResult`** — `{ diff: PlanDiff, daily_tasks_pruned, daily_tasks_materialized }`. Existing void-callers unaffected (TS allows ignoring return). After every replace, an entry lands in `plan_adjustments` (level=3, scope="plan", with diff stats). When `diff.isOverhaul` is true (>40% of tasks changed OR >10 absolute task changes) AND the goal is `planConfirmed`, a `proactive` nudge with body "X tasks added/removed/edited; N stale daily tasks cleared, M materialized" is emitted (idempotent context `plan-overhaul:${goalId}`).

(3) **`@starward/core.diffPlans(oldPlan, newPlan, thresholds?)`** — pure function exposing the diff math. Returns `PlanDiff { milestones: { added, removed, modified, totalOld, totalNew }, tasks: { added, removed, modified, totalOld, totalNew }, totalTaskChanges, isOverhaul }`. Comparison is by node `id` (stable-id edits report as `modified`, not `removed+added`). Excludes `completed`/`completedAt` from "modified" (those are progress, not edits).

(4) **`cmdCreateTask` priority policy changed** — manual user-created tasks (source `user_created`) now default to `priority="must-do"` (highest). They inherit triage's existing must-do shielding. Caller-supplied priority still wins (gap-filler proposals etc.). Non-user paths fall back to the prior auto-bonus-on-capacity rule.

(5) **`overwhelm` nudge from `lightTriage`** now carries a structured `extra.swapCandidate: { taskId, title, currentPriority, cognitiveCost, tier }` when the must-do shielding can't reach the cap. Body text names the candidate too. Mirrors the `swapSuggestion` shape from `cmdAcceptTaskProposal` and `cmdRescheduleTask`.

(6) **Daily mutation pipeline** (`services/dailyMutationPipeline.ts`) now serial: triage → estimator → deterministic auto-scheduler. New `services/dailyAutoScheduler.ts` places unscheduled tasks into the user's working-hours envelope (default 9–18 local) by triage order, never overwriting user manual placements (null-check on `scheduledStartIso`). LLM scheduler agent reserved for L1+ escalations to keep the $0.50/user/month target.

Earlier 2026-04-26 sections preserved below.

---

Last updated: 2026-04-26 — **Plan-adjustment escalation system (Phase A–E), past-day calendar contract, reminder check-off contract, daily-mutation pipeline.**

⚠ NEW WIRE SHAPES — read before building FE:

(1) **`command:request-escalation`** added — body `{ goalId?, milestoneId?, level: 1|2|3, reason: string, force?: boolean }`. Manual-override route into the L1/L2/L3 plan-adjustment handlers. L3 honors `goals.last_full_regen_at` 30-day cooldown unless `force=true`. Invalidates `view:dashboard | view:tasks | view:calendar | view:planning | view:goal-plan`. No FE wiring yet (deferred to post-notarization phase) — invokable via dev harness or chat intent.

(2) **`command:acknowledge-reminder` / `command:delete-reminder` / `command:delete-reminders-batch` now return 404** with envelope `{ ok: false, error: { code: "not_found", message: "reminder <id> not found for current user" } }` when the id doesn't match a reminder owned by the caller. Previously returned `{ ok: true }` on miss (silent failure — see git log 2026-04-26 for the symptom). The batch handler returns `{ ok: true, deletedCount, missingIds }` so a partial "Clear all" still completes for the rows that exist.

(3) **`view:tasks.pendingReschedules[]` shape extended** — each entry now carries `daysOverdue: number` and `agedOut: boolean` (true when the task is >30 days overdue). Tasks >90 days overdue are auto-skipped server-side via `markStaleAsSkipped` (run from `tasksView` resolver) with `payload.skippedReason = "aged-out"` — they no longer surface in this list.

(4) **`view:calendar.tasks` past-day contract** — past-day rows where `completed === false && skipped === false` are filtered out before return. Past days show only what was actually completed. Incomplete past tasks live on the Tasks page as `pendingReschedules` cards until rescheduled, dropped, or aged out. Same filter applies to `goalPlanTasks` and to virtual recurring instances generated by `expandRecurring` (past virtuals are now skipped — they were never real records).

(5) **Migrations 0014, 0015, 0016 added.**
- `0014_llm_calls.sql` — append-only ledger of every Anthropic SDK call. Cost computed in cents at insert via `core/llm-pricing.ts`. Populated by Proxy wrap of `getClient()` returned client (single injection point covers all ~20 SDK call sites).
- `0015_plan_adjustments.sql` — audit log of every escalation-classifier decision. Joined to `llm_calls` via `llm_call_ids[]` for cost attribution.
- `0016_last_full_regen_at.sql` — adds `goals.last_full_regen_at timestamptz`. L3 rate-limit gate (30-day cooldown unless `force=true`).

(6) **`core.classifyAdjustment(input)` added** alongside the existing `classifyReschedule`. Pure 4-level classifier (L0/L1/L2/L3) with a `scope: "task" | "day" | "milestone" | "plan"` axis. Returns `{ level, scope, rationale, rateLimited? }`. Inputs include `pendingReschedules.{count, daysOverdueSum, agedOutCount}`, `triage.undemoted`, `paceMismatch.maxSeverity`, `lowCompletionStreakDays`, `manualOverride?`, `goalLastFullRegenAt?`. The original 3-level `classifyReschedule` (goal-plan-only) stays for back-compat — additive.

(7) **Cron addition** — `fireMidnightRollover(userId, tz)` runs in the existing `backend/src/scheduler/index.ts` hourly tick at user-local 00:00. Body: invokes `services/planAdjustmentL0.runL0DayScope({date, force: true})`. Coexists with the lazy resolver-fetch sweep (`tasksView` calls `markStaleAsSkipped` when fetched). Idempotency via `daily_logs.payload.lastRolloverAt`.

(8) **Daily-mutation pipeline contract** — new `services/dailyMutationPipeline.ts` exports `fireDailyMutationPipeline(date | date[], kind)`. **Every command handler that creates/updates/deletes/completes/skips/reschedules a daily_task MUST dispatch this for the affected date(s).** Today: dispatches `fireLightTriage` + `fireEstimateDurations`. Wired into `cmdCreateTask`, `cmdToggleTask`, `cmdSkipTask`, `cmdDeleteTask`, `cmdDeleteTasksForDate`, `cmdUpdateTask`, `cmdRescheduleTask`. Was scattered ad-hoc per handler before — `cmdSkipTask` and `cmdUpdateTask` fired no downstream services, leaving stale calendar/tasks views. See `ARCHITECTURE_DAILY_PIPELINE.md` for the contract.

Earlier session (2026-04-26 morning): **Past-day rollover, calendar past-day filter, reminder check-off silent-failure fix, FE pendingReschedules card wiring.** See git log for details.
