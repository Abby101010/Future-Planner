/* NorthStar - Goal Breakdown Page (Phase 6b rewrite)
   Hierarchical drill-down: Year > Month > Week > Day
   With calendar integration and reallocation.

   Reads `view:goal-breakdown` via useQuery. Mutations go through
   useCommand: `command:regenerate-goal-plan` for generation, and
   `command:reallocate-goal-plan` for reallocation. No Zustand reads. */
...
export default function GoalBreakdownPage() {
  ...
  const { data, loading, error: queryError, refetch } =
    useQuery<GoalBreakdownViewModel>("view:goal-breakdown");
  ...
  // Decide whether to show the "new goal" form: if no breakdown yet, open it.
  const effectiveShowNewGoal = showNewGoal || !goalBreakdown;

  const isLoading = loading || commandRunning;
  const errorMessage =
    localError ??
    ...
    return (
      <div className="goal-breakdown">
        <div className="goal-breakdown-scroll">
          <div className="gb-loading animate-fade-in">
  ...
  return (
    <div className="goal-breakdown">
      <div className="goal-breakdown-scroll">
        ...
        {/* Breakdown Overview */}
        {goalBreakdown && !effectiveShowNewGoal && (
